...
\begin{itemize}
\item In a proportional limit, the number of layers $\(L\)$ and width $\(n\)$ satisfy $\(L/n \rightarrow d/w\)$ for some constant ratio $\(d/w\)$ as $\(n \rightarrow \infty\)$.
\item This constant ratio allows defining a continuous time $\(t = l/n \in [0, T]\)$ where $\(T = d/w\)$ and $\(l\)$ is the layer index.
\item As \(n \rightarrow \infty\), there are infinitesimally small gaps $\(1/n\)$ between discrete layers $\(l\)$ and $\(l+1\)$ in this pseudo-continuous time.
\item The attention operation gives covariance updates of size $\(O(1/n)\)$.
\item This means the discrete covariance updates $\(V_{l+1} - V_l\)$ are small $(\(O(1/n)\))$ for large $\(n\)$.
\item These small updates can be embedded into the continuous time \(t \in [0, T]\) as \(n \rightarrow \infty\).
\item The evolution of $\(V_l\)$ in discrete layers can be approximated by discretizing the SDE $\(dV_t\)$ using step size $\(1/n\)$.
\item This gives updates like $\(V_{l+1} \approx V_l + \text{(SDE drift and diffusion terms)}\)$ that converge to the continuous SDE.
\item So the constant $\(d/w\)$ ratio provides a continuous notion of time to embed discrete steps into.
\item The $\(O(1/n)\)$ update size allows approximating the SDE via these discrete updates.
\end{itemize}
